...
#
# Lexie Scholtz
# Created 2025.09.25 somewhere in the sea west of the Summer Isles, Scotland
...
# chis: [100], [80], [60], [40]
# rows:  ampure xp, biodynami lot 1, lot 2, lot 3
chis = [
...
biodynami_chis = []
for col in range(4):
    bdx = chis[1][col] + chis[2][col] + chis[3][col]
...
for j in range(4):
    ...
    res = ttest(ampure_chis[j], biodynami_chis[j])
    if res.pvalue < 0.05:
        print('*', end='')
    if res.pvalue < 0.005:
        print('*', end='')
...
for ax in axes:
    ax.set_xlim([-0.5, 2.17])
    ax.set_ylabel(r'$\chi_{eff}$')
    ax.set_ylim(yranges[0][0]-0.5, yranges[0][-1]+0.5)
...
biodynami_t = []
for col in range(4):
    bdt = resp_times[1][col] + resp_times[2][col] + resp_times[3][col]
    biodynami_t.append(bdt)

ampure_t = resp_times[0]

print('\n\nRESPONSE TIMES')
for j in range(4):
    conc = cs[j]
    print('for conc {} µg/mL:'.format(conc))
    res = ttest(ampure_t[j], biodynami_t[j])
    if res.pvalue < 0.05:
        print('*', end='')
    if res.pvalue < 0.005:
        print('*', end='')
...
img_name = '../subgraphics/chi_inter_biodynami_ampure_' + str(ver) + '.png'
